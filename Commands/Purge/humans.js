...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('humans')
    .setDescription('Deletes messages sent by human users (not bots).')
  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (filtered.size === 0) {
        return interaction.editReply(`⚠️ No messages from humans found in the last ${amount} messages.`);
      }

      const deleted = await interaction.channel.bulkDelete(filtered, true);

      return interaction.editReply(`🧹 Deleted **${deleted.size}** message(s) sent by human users.`);
    } catch (error) {
