...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('images')
    .setDescription('Deletes messages that contain images.')
  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (filtered.size === 0) {
        return interaction.editReply(`⚠️ No image messages found in the last ${amount} messages.`);
      }

      const deleted = await interaction.channel.bulkDelete(filtered, true);

      return interaction.editReply(`🧹 Deleted **${deleted.size}** message(s) containing images.`);
    } catch (error) {
      console.error('purge-images error:', error);
