...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('links')
    .setDescription('Deletes messages that contain links.')
  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (filtered.size === 0) {
        return interaction.editReply(`⚠️ No links found in the last ${amount} messages.`);
      }

      const deleted = await interaction.channel.bulkDelete(filtered, true);

      return interaction.editReply(`🧹 Deleted **${deleted.size}** message(s) containing links.`);
    } catch (error) {
